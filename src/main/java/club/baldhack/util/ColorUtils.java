...
public class ColorUtils {
    ...
    public String getColorNameFromRgb(int r, int g, int b) {
        ...
        Iterator<ColorName> iterator = colorList.iterator();
        do {
            if (!iterator.hasNext()) {
                if (closestMatch == null) return "No matched color name.";
                return closestMatch.getName();
            }
            ColorName c = iterator.next();
    ...
    public static int toRGBA(float[] colors) {
        if (colors.length == 4) return ColorUtils.toRGBA(colors[0], colors[1], colors[2], colors[3]);
        throw new IllegalArgumentException("colors[] must have a length of 4!");
    }

    public static int toRGBA(double[] colors) {
        if (colors.length == 4) return ColorUtils.toRGBA((float)colors[0], (float)colors[1], (float)colors[2], (float)colors[3]);
        throw new IllegalArgumentException("colors[] must have a length of 4!");
    }
